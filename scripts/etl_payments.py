import os
import pandas as pd
from sqlalchemy import create_engine

from dotenv import load_dotenv

load_dotenv()

DB_HOST = os.getenv("DB_HOST")
DB_NAME = os.getenv("DB_NAME")
DB_USER = os.getenv("DB_USER")
DB_PASSWORD = os.getenv("DB_PASSWORD")

DATABASE_URL = f"postgresql+psycopg2://{DB_USER}:{DB_PASSWORD}@{DB_HOST}/{DB_NAME}"
DATA_LAKE_PATH = "../DataLake"

def process_payments():
    print("Traitement de la table dim_payments...")
    payments = pd.read_csv(os.path.join(DATA_LAKE_PATH, "payments/olist_order_payments_dataset.csv"))
    
    # Agrégation par order_id
    payments_clean = payments.groupby('order_id', as_index=False).agg({
        'payment_sequential': 'max',
        'payment_type': lambda x: ','.join(set(x)),
        'payment_installments': 'sum',
        'payment_value': 'sum'
    })
    
    # Insertion dans PostgreSQL
    engine = create_engine(DATABASE_URL)
    payments_clean.to_sql('dim_payments', engine, if_exists='replace', index=False)
    print("dim_payments traitée et chargée avec succès.")

if __name__ == "__main__":
    process_payments()
